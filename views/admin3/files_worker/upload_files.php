...
<div class="row justify-content-center py-4 mx-0">
    ...
    <div class=" offset-1 col-10">
        <div class="card card-body bg-transparent border-0 shadow-sm">
            <form enctype="multipart/form-data" method="post" action="admin3.php?page=controllers/admin3/files_worker/upload_files_controller" class="needs-validation" novalidate>
                <div class="text-center my-4">
                    <span class="h3 font-weight-bold text-light"><?= language_content("Mettre un fichier en ligne", "Upload file") ?></span>
                </div>
                <div class="row mt-4 mb-0">
                    <div class="form-group col">
                        <label for="exampleInputEmail1"><?= language_content("Nom du fichier","File's Name")?></label>
                        <input type="text" name="name" class="form-control bg-dark text-info " required id="exampleInputEmail1" value="<?= $data['name'] ?? '' ?>" aria-describedby="">
                    </div>
                </div>
                <div class="row mt-0 mb-4">
                    <p>
                        <i class="fa fa-info-circle"></i> <?= language_content(" Si ce champ reste vide, le fichier gardera son nom par défaut", " if this input stay empty the file will keep her default name") ?>
                    </p>
                </div>
                ...
                <div class="row my-5">
                    <div class="custom-file col-8  mt-2">
                        ...
                        <label class="custom-file-label bg-dark text-info" for="file">
                            <?= isset($_FILES['files']['name']) ? $_FILES['files']['name'] : language_content("Choisir le fichier à uploader", "choose the file you want to upload") ?>
                        </label>
                        <p>
                            <i class="fa fa-info-circle"></i> <?= language_content(" les extensions pris en charge sont: jpeg, jpg, gif, png, doc, docx, xls, xlsx, pdf, zip, rar, pptx, pptm", " supported extensions are: jpeg, jpg, gif, png, doc, docx, xls, xlsx, pdf, zip, rar, pptx, pptm") ?>
                        </p>
                    </div>
                ...
                <div class="row justify-content-center">
                    <button type="submit" name="submit_upload_file" class="btn btn-outline-info col-md-4"><?= language_content("Mettre en ligne", "Upload") ?></button>
                </div>
            </form>
        </div>
    </div>
