...
if (isset($_POST['submit_add_direction'])) {
    ...
    if ($error == true) {
        require 'views/worker/directions/add_direction.php';
    } else {
...
if (isset($_POST['submit_name'])) {
    ...
    if (check_access_name($data1['name'], $db) === 0) {
        ...
        if (empty($data1['name']) || !is_string($data1['name']) || strlen($data1['name']) > 254) {
            $error = true;
            $error_content = what_langauge() == 0 ? "Nom incorect ou absent !" : "Incorrect or absent last name !";
            require 'views/worker/directions/add_direction_11.php';
        } else {
...
if (isset($_POST['submit_permission'])) {
    ...
    if ($error === true) {
        ...
        die('<meta http-equiv="refresh" content="0 ; URL=worker.php?page=views/worker/directions/add_direction">');
    }
